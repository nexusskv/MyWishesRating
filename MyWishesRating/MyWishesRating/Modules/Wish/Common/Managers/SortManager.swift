...
class SortManager {
    ...
    func displaySortView(_ view: WishViewController) {
    ...
    func sortByType(_ values: [String: AnyObject], view: WishViewController) {
    ...
    func sortActionHandler(_ view: WishViewController, value: Int) {
        if let type = WishesSortTypes(rawValue: value) {
            ...
            sortWishes(view, type: DataContainer.shared.selectedSortType)
        }
    }
    
    
    /// ---> Function for processing a sort data by selected type  <--- ///
    func sortWishes(_ view: WishViewController, type: WishesSortTypes) {
        if let array = view.viewModel.dataArray {
            var sortArray = array
